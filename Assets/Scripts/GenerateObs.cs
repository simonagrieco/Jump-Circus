...
public class GenerateObs : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		double r1 = Random.Range(2.5f, 6f);
		double r2 = Random.Range(2f, 5f);
		...
		if (t >= 120.0f)
		{
			...
			{
				random = Random.Range(0, 4);
				instanza= Instantiate(obs[random], pos, transform.rotation);
				instanza.transform.SetParent(transform.parent);
				Eight();
				t1 = 0.0f;
			}
		}
	...
	public float T1 { get { return t1; } set { t1 = value; } }
